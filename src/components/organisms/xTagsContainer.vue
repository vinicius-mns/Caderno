...
import TagOptions from './TagOptions.vue'
import TagsFilter from '../molecules/xTagsFilter.vue'
import TagsOptions from '../molecules/TagsOptions.vue'

const props = withDefaults(
  defineProps<{
    allTags: Itag[]
    filter?: { includeTags: Itag[]; excludeTags: Itag[] }
    tagsChecked: Itag[]
    textFilter: string
    type: 'tagsOptions' | 'filter' | 'selectable'
  }>(),
  {
...
const emit = defineEmits<{
  ...
  (e: 'getFilter', v: { includeTags: Itag[]; excludeTags: Itag[] }): void
}>()
...
const useFilter = () => {
  const currencentFilter = ref<{ includeTags: Itag[]; excludeTags: Itag[] }>(props.filter)

  const updateFilter = (newFilter: { includeTags: Itag[]; excludeTags: Itag[] }) => {
    currencentFilter.value = newFilter
  }

  const isNewFilter = computed(() => {
    // desestruturacao
    const { includeTags: cIncludeTags, excludeTags: cExcludeTags } = currencentFilter.value
    const { includeTags: pIncludeTags, excludeTags: pExcludeTags } = props.filter

    // comparar quantidades
    const crrQuantity = cIncludeTags.length + cExcludeTags.length
    const proQuantity = pIncludeTags.length + pExcludeTags.length

    if (crrQuantity !== proQuantity) return true

    // comparar objetos ordenados e transformados em string
    const crr = JSON.stringify({ i: [...cIncludeTags].sort(), e: [...cExcludeTags].sort() })
    const pro = JSON.stringify({ i: [...pIncludeTags].sort(), e: [...pExcludeTags].sort() })

    return crr !== pro
  })

  return {
    tags: currencentFilter,
    update: updateFilter,
    isNew: isNewFilter
  }
}
...
const openUpdateTag = (tag: Itag) => {
  emit('openUpdateTag', tag)
  closeModal()
}

const openDeleteTag = (tag: Itag) => {
  emit('openDeleteTag', tag)
  closeModal()
}

const sendNewFilter = () => {
  const { includeTags, excludeTags } = filterF.tags.value

  emit('getFilter', { includeTags, excludeTags })
}

// atualizacoes

onUpdated(() => filterF.update(props.filter))
</script>
  ...
  <FloatModalSlot ref="modal">
    ...
    <template #container-slot>
      <ModalCard class="modal-card-tags" background-color="front">
        ...
        <FlexContainer flex-wrap="wrap" class="tags-container" v-if="props.type === 'selectable'">
          <TagView2
        ...
        <TagsFilter
          ...
          :tags="props.allTags"
          :include-tags="props.filter.includeTags"
          :exclude-tags="props.filter.excludeTags"
          @get-filter="filterF.update"
        />

        <TagsOptions
          v-if="props.type === 'tagsOptions'"
          :tags="props.allTags"
        ...
        <ButtonSlot
          v-if="props.type === 'filter' && filterF.isNew.value"
          border-radius="50px"
